...
#include "../array/ArrayIO"
#include "../math/Math"
#include "../base/Meta"
#include "../base/StringTools"
...
template <typename T>
bool readSTLASCII(Geometry<T> &geometry, std::stringstream &f)
{
	typedef typename Geometry<T>::point_type	point_type;
	typedef typename Geometry<T>::vector_type	vector_type;
	typedef typename Geometry<T>::facet_type	facet_type;
...
template <typename T>
bool readSTLBinary(Geometry<T> &geometry, std::stringstream &f)
{
	typedef typename Geometry<T>::point_type	point_type;
	typedef typename Geometry<T>::vector_type	vector_type;
	typedef typename Geometry<T>::facet_type	facet_type;
...
template <typename T>
bool readFileSTL(Geometry<T> &geometry, const std::string fileName)
{
...
//This function was added as interface for string stl input
//This function converts the string into stringstream so that it can be used by the lower level overloaded functions identically
//Note that the binary input does not work for unknown reasons.
template <typename T>
bool readStringSTL(Geometry<T> &geometry, const std::string stringSTL)
{

	const std::string asciiHeader = "solid";
	std::stringstream ss(stringSTL);
	std::string buf;
	buf = stringSTL.substr(0,5);

	bool result = false;

	if (buf == asciiHeader)
	{
		result = detail::readSTLASCII (geometry, ss);
	}
	else
	{
		result = detail::readSTLBinary(geometry, ss);
	}
