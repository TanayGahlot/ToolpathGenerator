...
def distance(x,y, pt):
	return (x-pt[0])**2 + (y-pt[1])**2

def evenCloser(x,y, ptEven, ptOdd):
	if distance(x,y,ptEven) < distance(x,y, ptOdd):
		return True
	else:
		return False
...
def generateGcodeFromPath(paths, feed, jog, plunge):
	...
	for p in paths:
		...
		if evenCloser(x,y, p.points[0], p.points[-1]):
			...
			# Move to the start of this path at the jog height
			gcode += "G00 X%0.4f Y%0.4f Z%0.4f\n" %(p.points[-1][0], p.points[-1][1], jog)

			# Plunge to the desired depth
			gcode += "G01 Z%0.4f F%0.4f\n" % (p.points[-1][2], 60*scale*plunge)
